...
class Todaylist extends StatelessWidget {
  ...
  bool remove(Patientdetails patient) {
    ...
    if (today.day - selectedday.day == 1 &&
  ...
  List<Patientdetails> filteringtodaysList(List<Patientdetails> allpatients) {
    for (final patient in allpatients) {
  ...
  Widget build(BuildContext context) {
    filteringtodaysList(allpatients);
    ...
    if (filteredlist.isEmpty) {
      screen = const Padding(
        ...
        child: Center(child: Text("No patients for Today")),
      );
    }
    return Scaffold(
